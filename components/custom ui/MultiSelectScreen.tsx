...
interface MultiSelectProductProps {
  placeholder: string;
  productscreen: (ProductScreenType)[]; 
  value: string[];
...
const MultiSelectProduct: React.FC<MultiSelectProductProps> = ({
  ...
  const selected = value.map((id) =>
    productscreen.find((item) => item && item._id === id)
  ).filter((item): item is ProductScreenType  => !!item);
  

  const selectables = productscreen.filter((item) => !selected.includes(item));
